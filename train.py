...
if __name__ == "__main__":
    ...
    method = get_method_from_args(args)
    args_dict = args_to_dict(args)
    name = f"{args.env_name}-{method}-{str(args.seed)}"
    args.xpid = name
    ...
    if args.use_wandb:
        ...
        wandb.init(
            ...
            name=args.version,
            config=args_dict,
            tags=[args.version, method, args.env_name],
        )
    ...
    device = torch.device(f"cuda:{cuda_num}" if args.cuda else "cpu")
    ...
    if args.use_plr and not args.use_reset_random_dr:
